...
#include <unistd.h>
#include <sys/types.h>
#define COLUMN_NAME_SIZE 255
#define COLUMN_USERNAME_SIZE 32
#define COLUMN_EMAIL_SIZE 255
#define COLUMN_PASSWORD_SIZE 32
...
const uint32_t USERNAME_OFFSET = EMAIL_OFFSET + EMAIL_SIZE;
const uint32_t PASSWORD_OFFSET = USERNAME_OFFSET + USERNAME_SIZE;
const uint32_t ROW_SIZE = ID_SIZE + NAME_SIZE + EMAIL_SIZE + USERNAME_SIZE + PASSWORD_SIZE;
...
PrepareResult prepare_insert(InputBuffer* input_buffer, Statement* statement) {
  ...
  strcpy(statement->row_to_insert.password, password);
...
void print_row(Row* row) {
  printf("(%d, %s, %s, %s, %s)\n", row->id, row->name, row->email, row->username, row->password);
}
